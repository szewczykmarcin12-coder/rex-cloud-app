...
import { Calendar, Home, Umbrella, Clock, Menu, X, ChevronLeft, ChevronRight, LogOut, Info, User, Plus, Cloud, Download } from 'lucide-react';
...
const ShiftsPage = ({ date, onDateChange, shifts }) => {
  ...
  const filtered = shifts
    .filter(s => { const d = new Date(s.date); return (!selectedDay || d.getDate() === selectedDay) && d.getMonth() === date.getMonth() && d.getFullYear() === date.getFullYear(); })
...
function REXCloudApp() {
  ...
  const [user, setUser] = useState({ name: 'MARCIN SZEWCZYK', initials: 'MS', email: 'szewczyk.marcin12@gmail.com', company: 'Rex Concepts', phone: '+48 123 456 789', address: 'Kraków' });
  ...
  // Auto save when shifts change
  useEffect(() => { 
    if (logged && calendarSha && shifts.length > 0) {
      const timer = setTimeout(() => saveToGitHub(), 1000);
  ...
  const saveToGitHub = async () => {
    try {
      ...
      const res = await fetch(API_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ content: icsContent, sha: calendarSha, message: 'Auto-sync from REX Cloud' }) });
      const data = await res.json();
