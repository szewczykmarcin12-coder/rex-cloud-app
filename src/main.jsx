...
import { Calendar, Home, Umbrella, Clock, Menu, X, ChevronLeft, ChevronRight, LogOut, Info, User, Plus, Cloud, TrendingUp, DollarSign, BarChart3, Lock, AlertCircle } from 'lucide-react';
...
const positionColors = { 
  ...
  'LOB': '#64748B',  // Szary - Lobby
  'TRA': '#6B7280',  // Szary - Training
  'MGR': '#1E3A8A'   // Nasycony szafir - Manager
};
...
const colors = {
  // Szafirowe (główne)
  primary: {
    darkest: '#082567',  // Najciemniejszy - nagłówki, gradienty
    dark: '#213b76',     // Ciemny - hover, akcenty
    medium: '#395185',   // Średni - główny kolor
    light: '#526695',    // Jasny - tekst, ikony
    bg: '#e8edf5',       // Tło jasne
    bgLight: '#f1f4f9'   // Tło bardzo jasne
  },
  // Brzoskwiniowe (akcenty - urlopy, wynagrodzenia)
  accent: {
    dark: '#FDA785',     // Ciemniejszy brzoskwiniowy
    medium: '#FFBF99',   // Średni
    light: '#FBCEB1',    // Jasny
    bg: '#FFF5EE'        // Tło
  }
const positionNames = {
...
const generateICS = (shifts) => {
  ...
  const events = shifts.map(s => { 
    const [st, et] = s.shifts[0].time.split(' - '); 
    const [sh, sm] = st.split(':'); 
    const [eh, em] = et.split(':'); 
    const start = new Date(s.date); start.setHours(+sh, +sm); 
    const end = new Date(s.date); end.setHours(+eh, +em); 
    const uid = typeof s.id === 'string' && s.id.includes('@') ? s.id : 'shift-'+s.id+'@rexcloud.app'; 
    return 'BEGIN:VEVENT\nUID:'+uid+'\nDTSTAMP:'+fmt(new Date())+'\nDTSTART:'+fmt(start)+'\nDTEND:'+fmt(end)+'\nSUMMARY:'+s.shifts[0].type+' - Popeyes PLK\nLOCATION:'+s.location+'\nSTATUS:CONFIRMED\nEND:VEVENT'; 
  }).join('\n');
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-2xl p-8">
          ...
          <div className="space-y-4">
            <div><label className="block text-sm text-slate-600 mb-1">Email</label><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-4 py-3 rounded-xl border focus:outline-none" style={{'--tw-ring-color': colors.primary.medium}} placeholder="Email" disabled={loading} /></div>
            <div><label className="block text-sm text-slate-600 mb-1">PIN</label><input type="password" value={pin} onChange={(e) => setPin(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleLogin()} className="w-full px-4 py-3 rounded-xl border focus:outline-none" placeholder="••••" maxLength={4} disabled={loading} /></div>
      ...
      <div className="grid grid-cols-7 gap-1 p-2">
        ...
        {days.map((item, i) => { 
          ...
          return (
            ...
            >
            <button key={i} onClick={() => item.current && onDayClick(item.day === selectedDay ? null : item.day)} 
              className={`flex flex-col items-center py-2 rounded-full ${!item.current ? 'text-slate-300' : selectedDay === item.day ? 'bg-cyan-100 text-cyan-700' : isToday(item.day) ? 'bg-cyan-500 text-white' : 'text-slate-700'}`}>
...
const Sidebar = ({ isOpen, onClose, currentPage, onNavigate, user, onLogout }) => {
  ...
  const items = [
    { id: 'home', icon: Home, label: 'Strona domowa' }, 
    { id: 'shifts', icon: Calendar, label: 'Zmiany' }, 
    { id: 'holidays', icon: Umbrella, label: 'Czas wolny' }, 
    { id: 'workedTime', icon: Clock, label: 'Przepracowany Czas' }, 
    { id: 'userData', icon: User, label: 'Dane użytkownika' }, 
    { id: 'about', icon: Info, label: 'O Aplikacji' }
  ];
  return (
    <>
      ...
      <div className={`fixed top-0 left-0 h-full w-72 bg-white z-50 transform transition-transform flex flex-col ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="bg-gradient-to-r from-cyan-500 to-cyan-600 p-4 pt-8">
          <div className="flex items-center gap-2 mb-4"><Cloud size={24} className="text-white" /><span className="text-white text-lg font-light">REX <span className="text-cyan-200">Cloud</span></span></div>
        </div>
        <div className="p-4 border-b flex items-center gap-3">
          <div className="w-12 h-12 rounded-full flex items-center justify-center text-white font-semibold" style={{backgroundColor: colors.primary.medium}}>{user.initials}</div>
          <div className="w-12 h-12 bg-cyan-500 rounded-full flex items-center justify-center text-white font-semibold">{user.initials}</div>
          <div><p className="font-semibold text-sm">{user.name}</p><p className="text-slate-500 text-xs">{user.email}</p></div>
        </div>
        <nav className="p-4 flex-1">
          {items.map(item => (
            <button key={item.id} onClick={() => { onNavigate(item.id); onClose(); }} className="w-full flex items-center gap-4 px-4 py-3 rounded-xl" style={currentPage === item.id ? {backgroundColor: colors.primary.bg, color: colors.primary.dark} : {color: '#475569'}}>
            <button key={item.id} onClick={() => { onNavigate(item.id); onClose(); }} 
              className={`w-full flex items-center gap-4 px-4 py-3 rounded-xl ${currentPage === item.id ? 'bg-cyan-50 text-cyan-600' : 'text-slate-600'}`}>
              <item.icon size={20} /><span className="font-medium">{item.label}</span>
            </button>
          ))}
        </nav>
        <div className="p-4 border-t">
          <button onClick={() => { onLogout(); onClose(); }} className="w-full flex items-center gap-4 px-4 py-3 rounded-xl text-red-600"><LogOut size={20} /><span className="font-medium">Wyloguj się</span></button>
          <button onClick={() => { onLogout(); onClose(); }} className="w-full flex items-center gap-4 px-4 py-3 rounded-xl text-red-600">
            <LogOut size={20} /><span className="font-medium">Wyloguj się</span>
          </button>
        </div>
      </div>
    </>
...
const Header = ({ title, onMenuClick }) => (
  ...
  <div className="bg-gradient-to-r from-cyan-500 to-cyan-600 text-white px-4 py-4 flex items-center justify-between sticky top-0 z-30">
...
const SwipeableShiftCard = ({ shift, onShowCoworkers, isRevealed, onReveal, isSelected, onSelect }) => {
  ...
  const handleTouchStart = (e) => { startX.current = e.touches[0].clientX; hasMoved.current = false; setIsDragging(true); };
  const handleTouchMove = (e) => { if (!isDragging) return; currentX.current = e.touches[0].clientX; const diff = startX.current - currentX.current; if (Math.abs(diff) > 5) hasMoved.current = true; if (diff > 0) { setTranslateX(Math.min(diff, 80)); } else { setTranslateX(0); } };
  const handleTouchEnd = () => { setIsDragging(false); if (translateX > 40) { setTranslateX(70); onReveal(shift.id); } else { setTranslateX(0); if (!hasMoved.current) { onSelect(shift.id); } } };
  const handleMouseDown = (e) => { startX.current = e.clientX; hasMoved.current = false; setIsDragging(true); };
  const handleMouseMove = (e) => { if (!isDragging) return; currentX.current = e.clientX; const diff = startX.current - currentX.current; if (Math.abs(diff) > 5) hasMoved.current = true; if (diff > 0) { setTranslateX(Math.min(diff, 80)); } else { setTranslateX(0); } };
  const handleMouseUp = () => { setIsDragging(false); if (translateX > 40) { setTranslateX(70); onReveal(shift.id); } else { setTranslateX(0); if (!hasMoved.current) { onSelect(shift.id); } } };
  const handleMouseLeave = () => { if (isDragging) { setIsDragging(false); if (translateX > 40) { setTranslateX(70); onReveal(shift.id); } else { setTranslateX(0); } } };
  ...
  return (
    <div className="relative overflow-hidden rounded-xl shadow-sm mb-3">
      <div className="absolute right-0 top-0 bottom-0 w-20 flex items-center justify-center rounded-r-xl" style={{backgroundColor: colors.primary.medium}}>
        <button onClick={() => onShowCoworkers(shift)} className="w-full h-full flex items-center justify-center">
      ...
      <div 
        ...
        style={{ 
          transform: `translateX(-${isRevealed ? 70 : translateX}px)`,
          backgroundColor: isHighlighted ? colors.primary.bg : 'white',
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center">
      <div className="bg-slate-50 w-full max-w-lg rounded-t-3xl max-h-[85vh] overflow-hidden flex flex-col">
        <div className="bg-white p-4 border-b"><h3 className="text-xl font-semibold text-slate-700">Współpracownicy ze zmiany</h3></div>
        <div className="flex-1 overflow-y-auto p-4">
          <div className="bg-white rounded-2xl p-4 mb-4 shadow-sm">
            <div className="flex items-center gap-3 mb-3"><div className="w-10 h-10 bg-slate-100 rounded-full flex items-center justify-center"><MapPin size={20} className="text-slate-500" /></div><span className="text-slate-700">{shift.location}</span></div>
            <div className="flex items-center gap-3 mb-3"><div className="w-10 h-10 bg-slate-100 rounded-full flex items-center justify-center"><Calendar size={20} className="text-slate-500" /></div><span className="text-slate-700">{formattedDate}</span></div>
            <div className="flex items-center gap-3"><div className="w-10 h-10 bg-slate-100 rounded-full flex items-center justify-center"><Clock size={20} className="text-slate-500" /></div><span className="text-slate-700">{shift.shifts[0].time}</span></div>
          </div>
          <div className="bg-white rounded-2xl p-4 shadow-sm">
            <h4 className="text-lg font-semibold text-slate-700 mb-4">Współpracownicy ze zmiany</h4>
            {coworkers.length === 0 ? (
              <div className="text-center py-8"><Users size={48} className="text-slate-200 mx-auto mb-3" /><p className="text-slate-400">Brak informacji o współpracownikach</p></div>
            ) : (
              <div className="space-y-3">{coworkers.map((cw, i) => (<div key={i} className="flex items-center gap-3 py-2"><div className="w-12 h-12 bg-slate-200 rounded-full flex items-center justify-center"><User size={24} className="text-slate-400" /></div><div><p className="font-semibold text-slate-700">{cw.name}</p><p className="text-sm" style={{color: colors.primary.medium}}>{cw.time}</p></div></div>))}</div>
            )}
          </div>
        </div>
        <div className="p-4 bg-white border-t">
          <button onClick={onClose} className="w-full py-3 text-white font-semibold rounded-xl" style={{backgroundColor: colors.primary.medium}}>Ok</button>
        </div>
      </div>
    </div>
  );
...
// ZAKTUALIZOWANA HomePage - automatyczne daty
const HomePage = ({ nextShift, onNavigateToShifts, vacation, onNavigateToHolidays }) => {
  ...
  const calcCountdown = (dateStr, time) => { 
    if (!dateStr) return { days: 0, hours: 0, min: 0 }; 
    const target = new Date(dateStr); 
    if (time) { const [h, m] = time.split(':'); target.setHours(+h, +m); } 
  ...
  return (
    <div className="p-4 space-y-4 pb-24">
      ...
      <div className="bg-white rounded-2xl shadow-sm border-l-4 border-cyan-500 p-4 cursor-pointer" onClick={onNavigateToShifts}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold">Następna zmiana</h3>
          <Calendar size={24} style={{color: colors.primary.medium}} />
          <Calendar size={24} className="text-cyan-500" />
        </div>
        {nextShift ? (
          <div className="flex gap-4">
            <div className="rounded-xl p-3 text-center min-w-16" style={{backgroundColor: colors.primary.bg}}>
              <p className="text-sm" style={{color: colors.primary.light}}>{nextShift.dayName}</p>
            <div className="bg-slate-50 rounded-xl p-3 text-center min-w-16">
              <p className="text-sm text-slate-500">{nextShift.dayName}</p>
              <p className="text-3xl font-bold">{nextShift.dayNum}</p>
              <p className="text-sm" style={{color: colors.primary.light}}>{monthNames[new Date(nextShift.date).getMonth()].substring(0,3).toUpperCase()}</p>
              <p className="text-sm text-slate-500">{monthNames[new Date(nextShift.date).getMonth()].substring(0,3).toUpperCase()}</p>
            </div>
            <div className="flex-1">
              ...
              <div className="flex gap-4 mt-4 pt-3 border-t">
                ...
                <div className="text-center"><p className="text-2xl font-bold text-cyan-500">{shiftCountdown.days}</p><p className="text-xs text-slate-500">Dni</p></div>
          ...
          <div className="text-center py-4">
            <Cloud size={40} className="text-slate-200 mx-auto mb-2" />
            <p className="text-slate-500">Brak zaplanowanych zmian</p>
          </div>
        )}
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border-l-4 border-amber-400 p-4 cursor-pointer" onClick={onNavigateToHolidays}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold">Następny urlop</h3>
          <Umbrella size={24} style={{color: colors.accent.dark}} />
          <Umbrella size={24} className="text-amber-500" />
        </div>
        {vacation ? (
          <div className="flex gap-4">
            <div className="rounded-xl p-3 text-center min-w-16" style={{backgroundColor: colors.accent.bg}}>
              <p className="text-sm" style={{color: colors.accent.dark}}>{vacDayNames[vacDate.getDay()]}</p>
            <div className="bg-amber-50 rounded-xl p-3 text-center min-w-16">
              <p className="text-sm text-amber-600">{vacDayNames[vacDate.getDay()]}</p>
              <p className="text-3xl font-bold">{vacDate.getDate()}</p>
              <p className="text-sm" style={{color: colors.accent.dark}}>{monthNames[vacDate.getMonth()].substring(0,3).toUpperCase()}</p>
              <p className="text-sm text-amber-600">{monthNames[vacDate.getMonth()].substring(0,3).toUpperCase()}</p>
            </div>
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1"><Umbrella size={16} style={{color: colors.accent.dark}} /><span className="font-medium">{vacation.type}</span></div>
              <div className="flex items-center gap-2 mb-1"><Umbrella size={16} className="text-amber-500" /><span className="font-medium">{vacation.type}</span></div>
              <div className="flex items-center gap-2 text-slate-600 text-sm"><Calendar size={14} className="text-slate-400" /><span>{formatDate(vacDate)} - {formatDate(vacEndDate)}</span></div>
              <p className="text-sm font-medium mt-1" style={{color: colors.accent.dark}}>Łącznie: {vacation.totalDays} dni</p>
              <p className="text-amber-600 text-sm font-medium mt-1">Łącznie: {vacation.totalDays} dni</p>
              <div className="flex gap-4 mt-4 pt-3 border-t">
                <div className="text-center"><p className="text-2xl font-bold" style={{color: colors.accent.dark}}>{vacCountdown.days}</p><p className="text-xs text-slate-500">Dni</p></div>
                <div className="text-center"><p className="text-2xl font-bold text-amber-500">{vacCountdown.days}</p><p className="text-xs text-slate-500">Dni</p></div>
          ...
          <div className="text-center py-4"><Umbrella size={40} style={{color: colors.accent.light}} className="mx-auto mb-2" /><p className="text-slate-500">Brak zaplanowanych urlopów</p></div>
          <div className="text-center py-4">
            <Umbrella size={40} className="text-amber-200 mx-auto mb-2" />
            <p className="text-slate-500">Brak zaplanowanych urlopów</p>
          </div>
        )}
      </div>
    </div>
...
// ZAKTUALIZOWANA ShiftsPage - automatyczne miesiące
const ShiftsPage = ({ date, onDateChange, shifts }) => {
  ...
  // Automatycznie ustaw dzień na dziś przy pierwszym renderze
  useEffect(() => {
    const today = new Date();
    if (date.getMonth() === today.getMonth() && date.getFullYear() === today.getFullYear()) {
      // Znajdź najbliższą zmianę od dziś
      const todayStr = today.toISOString().split('T')[0];
      const futureShift = shifts.find(s => s.date >= todayStr);
      if (futureShift) {
        setSelectedDay(new Date(futureShift.date).getDate());
      } else {
  ...
  const handleReveal = (shiftId) => { setRevealedShiftId(shiftId); setClickedShiftId(null); };
  const handleSelect = (shiftId) => { setClickedShiftId(prev => prev === shiftId ? null : shiftId); setRevealedShiftId(null); };
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50 pb-20">
      ...
      <div className="flex-1 p-4 space-y-3">
        {filtered.length === 0 ? (
          <div className="text-center py-12"><Cloud size={48} className="text-slate-300 mx-auto mb-4" /><p className="text-slate-500">Brak zmian w tym okresie</p></div>
        ) : (
          filtered.map(shift => (
            <SwipeableShiftCard key={shift.id} shift={shift} onShowCoworkers={setSelectedShift} isRevealed={revealedShiftId === shift.id} onReveal={handleReveal} isSelected={clickedShiftId === shift.id} onSelect={handleSelect} />
          ))
        )}
          ...
          <div key={shift.id} className="bg-white rounded-xl shadow-sm border-l-4 border-cyan-400 p-4">
            <div className="flex gap-4">
              <div className="bg-cyan-50 rounded-xl px-3 py-2 text-center min-w-14">
              ...
              <div className="flex-1">
                {shift.shifts.map((s,i) => (
                  <div key={i} className="flex items-center gap-2 mb-1">
                    <span>{s.time}</span>
                    <span className="w-2 h-2 rounded-full" style={{backgroundColor: s.color}} />
                    <span className="text-sm text-slate-600">{s.type} - {positionNames[s.type] || s.type}</span>
                  </div>
                ))}
                <p className="text-slate-500 text-sm mt-1">{shift.location}</p>
              </div>
      ...
      {selectedShift && <CoworkersModal shift={selectedShift} onClose={() => setSelectedShift(null)} />}
    </div>
  );
};
...
// ZAKTUALIZOWANA PreferencesPage - wnioski NIE przekładają się na realne zmiany
const PreferencesPage = ({ date, onDateChange, requests, onAddRequest }) => {
  ...
  const positions = [{ id: 'KIT', name: 'KIT - Kuchnia' }, { id: 'CAS', name: 'CAS - Kasa' }, { id: 'SUP', name: 'SUP - Wsparcie' }, { id: 'RUN', name: 'RUN - Runner' }, { id: 'SIN', name: 'SIN - Sink' }, { id: 'LOB', name: 'LOB - Lobby' }];
  
  // Zaktualizowana lista pozycji z TRA i MGR
  const positions = [
    ...
    { id: 'TRA', name: 'TRA - Training' },
    { id: 'MGR', name: 'MGR - Manager' }
  ];
  ...
  const handleAdd = () => {
    if (newPref.date < todayStr) { setError('Nie można dodać zmiany w przeszłości'); return; }
    setError('');
    const d = new Date(newPref.date); const dayNamesShort = ['NI', 'PO', 'WT', 'ŚR', 'CZ', 'PT', 'SO']; const dayNamesFullArr = ['NIEDZ', 'PON', 'WT', 'ŚR', 'CZW', 'PT', 'SOB'];
  ...
  const handleAdd = () => { 
    ...
    const request = { 
      ...
      status: 'pending', // Nowy status - oczekuje na zatwierdzenie
      createdAt: new Date().toISOString()
    }; 
    
    onAddRequest(request);
    setShowModal(false); 
    setNewPref({ date: new Date().toISOString().split('T')[0], type: 'Pracuj', position: 'KIT', timeFrom: '08:00', timeTo: '16:00' }); 
  };

  // Filtruj wnioski według wybranego miesiąca
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50 pb-20">
      <div className="bg-white px-4 py-4 border-b flex items-center justify-between">
        <button onClick={() => onDateChange(new Date(date.getFullYear(), date.getMonth()-1, 1))} className="p-2"><ChevronLeft size={24} /></button>
        <span className="font-semibold">{monthNames[date.getMonth()]} {date.getFullYear()}</span>
        <button onClick={() => onDateChange(new Date(date.getFullYear(), date.getMonth()+1, 1))} className="p-2"><ChevronRight size={24} /></button>
      </div>
      ...
      <div className="p-4">
        ...
        {prefs.length === 0 ? (
          <div className="text-center py-12"><Cloud size={48} className="text-slate-300 mx-auto mb-4" /><p className="text-slate-500">Brak wniosków</p></div>
        ) : prefs.map(p => (
          <div key={p.id} className="bg-white rounded-xl shadow-sm p-4 mb-3" style={{borderLeft: `4px solid ${colors.primary.medium}`}}>
        <button onClick={() => setShowModal(true)} className="flex items-center gap-2 text-cyan-600 font-medium mb-4">
          <Plus size={20} /><span className="underline">Nowy wniosek o zmianę</span>
        </button>
        
        {filteredRequests.length === 0 ? (
          <div className="text-center py-12">
            <Cloud size={48} className="text-slate-300 mx-auto mb-4" />
            <p className="text-slate-500">Brak wniosków w tym miesiącu</p>
          </div>
        ) : filteredRequests.map(p => (
          <div key={p.id} className="bg-white rounded-xl shadow-sm border-l-4 border-amber-400 p-4 mb-3">
            <div className="flex items-center gap-4">
              <div className="rounded-xl px-3 py-2 text-center min-w-14" style={{backgroundColor: colors.primary.bg}}>
                <p className="text-xs" style={{color: colors.primary.light}}>{p.dayName}</p>
              <div className="bg-amber-50 rounded-xl px-3 py-2 text-center min-w-14">
                <p className="text-xs text-amber-600">{p.dayName}</p>
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  <span className="text-xs text-amber-600 font-medium">Oczekuje na zatwierdzenie</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center">
          <div className="bg-white w-full max-w-lg rounded-t-3xl p-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div className="p-3 rounded-xl text-sm" style={{backgroundColor: colors.primary.bg, color: colors.primary.dark}}>✓ Zmiana zostanie automatycznie zsynchronizowana</div>
              <div>
                <label className="block text-sm font-medium text-cyan-600 mb-2">Data</label>
                <input type="date" value={newPref.date} onChange={(e) => setNewPref({...newPref, date: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl" />
              </div>
              <div>
                <label className="block text-sm font-medium text-cyan-600 mb-2">Rodzaj</label>
                <select value={newPref.type} onChange={(e) => setNewPref({...newPref, type: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl">
                  {types.map(t => <option key={t} value={t}>{t}</option>)}
                </select>
              </div>
              {needsFrom && (
                <div>
                  <label className="block text-sm font-medium text-cyan-600 mb-2">Od godziny</label>
                  <input type="time" value={newPref.timeFrom} onChange={(e) => setNewPref({...newPref, timeFrom: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl" />
                </div>
              )}
              {needsTo && (
                <div>
                  <label className="block text-sm font-medium text-cyan-600 mb-2">Do godziny</label>
                  <input type="time" value={newPref.timeTo} onChange={(e) => setNewPref({...newPref, timeTo: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl" />
                </div>
              )}
              {needsPos && (
                <div>
                  <label className="block text-sm font-medium text-cyan-600 mb-2">Pozycja</label>
                  <select value={newPref.position} onChange={(e) => setNewPref({...newPref, position: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl">
              ...
              <div className="bg-amber-50 p-3 rounded-xl text-sm text-amber-700 flex items-center gap-2">
                <AlertCircle size={18} />
                <span>Wniosek zostanie przesłany do administratora i będzie oczekiwał na zatwierdzenie</span>
              </div>
            </div>
            <div className="flex gap-3 mt-6">
              <button onClick={() => setShowModal(false)} className="flex-1 py-3 bg-slate-100 rounded-xl font-semibold">Anuluj</button>
              <button onClick={handleAdd} className="flex-1 py-3 text-white rounded-xl font-semibold" style={{backgroundColor: colors.primary.medium}}>Dodaj zmianę</button>
              <button onClick={handleAdd} className="flex-1 py-3 bg-cyan-500 text-white rounded-xl font-semibold">Złóż wniosek</button>
            </div>
          </div>
        </div>
      )}
...
const HolidaysPage = ({ vacation, onAddVacation }) => {
  ...
  const handleAdd = () => { 
    if (newVac.startDate && newVac.endDate) { 
      const start = new Date(newVac.startDate); 
      const end = new Date(newVac.endDate); 
      const days = Math.ceil((end - start) / 86400000) + 1; 
      onAddVacation({ startDate: newVac.startDate, endDate: newVac.endDate, type: newVac.type, totalDays: days }); 
      setShowModal(false); 
      setNewVac({ startDate: '', endDate: '', type: 'Urlop wypoczynkowy' }); 
    } 
  };
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50 pb-24">
      <div className="p-4">
        ...
        <button onClick={() => setShowModal(true)} className="flex items-center gap-2 text-amber-600 font-medium mb-4">
          <Plus size={20} /><span className="underline">Dodaj nowy urlop</span>
        </button>
        {vacation ? (
          ...
          <div className="bg-white rounded-2xl shadow-sm border-l-4 border-amber-400 p-4">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold">Zaplanowany urlop</h3>
              <Umbrella size={24} className="text-amber-500" />
            </div>
            <div className="flex gap-4">
              <div className="rounded-xl p-3 text-center min-w-16" style={{backgroundColor: colors.accent.bg}}>
          ...
          <div className="bg-white rounded-2xl shadow-sm p-8 text-center">
            <Umbrella size={64} style={{color: colors.accent.light}} className="mx-auto mb-4" />
            <Umbrella size={64} className="text-amber-200 mx-auto mb-4" />
            <p className="text-slate-500 mb-4">Brak zaplanowanych urlopów</p>
            <button onClick={() => setShowModal(true)} className="px-6 py-3 text-white font-semibold rounded-xl" style={{backgroundColor: colors.accent.dark}}>Złóż wniosek</button>
            <button onClick={() => setShowModal(true)} className="px-6 py-3 bg-amber-500 text-white font-semibold rounded-xl">Złóż wniosek</button>
          </div>
        )}
      </div>
      {showModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center">
          <div className="bg-white w-full max-w-lg rounded-t-3xl p-6">
            ...
            <div className="space-y-4">
              <div><label className="block text-sm font-medium mb-2" style={{color: colors.accent.dark}}>Typ urlopu</label><select value={newVac.type} onChange={(e) => setNewVac({...newVac, type: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl">{vacTypes.map(t => <option key={t} value={t}>{t}</option>)}</select></div>
              <div><label className="block text-sm font-medium mb-2" style={{color: colors.accent.dark}}>Data rozpoczęcia</label><input type="date" min={todayStr} value={newVac.startDate} onChange={(e) => setNewVac({...newVac, startDate: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl" /></div>
              <div><label className="block text-sm font-medium mb-2" style={{color: colors.accent.dark}}>Data zakończenia</label><input type="date" min={newVac.startDate || todayStr} value={newVac.endDate} onChange={(e) => setNewVac({...newVac, endDate: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl" /></div>
              {newVac.startDate && newVac.endDate && (<div className="p-3 rounded-xl text-sm" style={{backgroundColor: colors.accent.bg, color: colors.accent.dark}}>Łączna liczba dni: <strong>{Math.ceil((new Date(newVac.endDate) - new Date(newVac.startDate)) / 86400000) + 1}</strong></div>)}
              <div><label className="block text-sm font-medium text-amber-600 mb-2">Typ urlopu</label><select value={newVac.type} onChange={(e) => setNewVac({...newVac, type: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl">{vacTypes.map(t => <option key={t} value={t}>{t}</option>)}</select></div>
              <div><label className="block text-sm font-medium text-amber-600 mb-2">Data rozpoczęcia</label><input type="date" value={newVac.startDate} onChange={(e) => setNewVac({...newVac, startDate: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl" /></div>
              <div><label className="block text-sm font-medium text-amber-600 mb-2">Data zakończenia</label><input type="date" value={newVac.endDate} onChange={(e) => setNewVac({...newVac, endDate: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl" /></div>
              {newVac.startDate && newVac.endDate && (<div className="bg-amber-50 p-3 rounded-xl text-sm text-amber-700">Łączna liczba dni: <strong>{Math.ceil((new Date(newVac.endDate) - new Date(newVac.startDate)) / 86400000) + 1}</strong></div>)}
            </div>
            <div className="flex gap-3 mt-6">
              <button onClick={() => setShowModal(false)} className="flex-1 py-3 bg-slate-100 rounded-xl font-semibold">Anuluj</button>
              <button onClick={handleAdd} className="flex-1 py-3 text-white rounded-xl font-semibold" style={{backgroundColor: colors.accent.dark}}>Złóż wniosek</button>
              <button onClick={handleAdd} className="flex-1 py-3 bg-amber-500 text-white rounded-xl font-semibold">Złóż wniosek</button>
            </div>
          </div>
        </div>
      )}
...
const StatisticsPage = ({ shifts, hourlyRate = 0 }) => {
  ...
  const monthsData = useMemo(() => { 
    ...
    for (let i = 2; i >= 0; i--) { 
      ...
      const monthShifts = shifts.filter(s => { const d = new Date(s.date); return d.getMonth() === month && d.getFullYear() === year; }); 
      let totalHours = 0, shiftsCount = 0; 
      // Zaktualizowane pozycje
      const positions = { KIT: 0, CAS: 0, SUP: 0, RUN: 0, SIN: 0, LOB: 0, TRA: 0, MGR: 0 }; 
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 pb-24 space-y-4">
      <div className="bg-white rounded-2xl shadow-sm p-4">
        <div className="flex items-center justify-between mb-4"><h3 className="text-lg font-semibold">{currentMonthData.fullLabel} {currentMonthData.year}</h3><Clock size={24} style={{color: colors.primary.medium}} /></div>
        <div className="flex items-center justify-between mb-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="rounded-xl p-4 text-center" style={{backgroundColor: colors.primary.bg}}><p className="text-3xl font-bold" style={{color: colors.primary.dark}}>{currentMonthData.totalHours.toFixed(1)}</p><p className="text-sm" style={{color: colors.primary.light}}>godzin</p></div>
          <div className="rounded-xl p-4 text-center" style={{backgroundColor: colors.primary.bg}}><p className="text-3xl font-bold" style={{color: colors.primary.dark}}>{currentMonthData.shiftsCount}</p><p className="text-sm" style={{color: colors.primary.light}}>zmian</p></div>
        ...
        {hourlyRate > 0 && (
          <div className="mt-4 rounded-xl p-4" style={{backgroundColor: colors.accent.bg}}>
          <div className="mt-4 bg-amber-50 rounded-xl p-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2"><DollarSign size={20} style={{color: colors.accent.dark}} /><span className="text-sm" style={{color: colors.accent.dark}}>Prognozowane wynagrodzenie</span></div>
              <span className="text-xl font-bold" style={{color: colors.accent.dark}}>{currentMonthData.earnings.toFixed(2)} zł</span>
      ...
      <div className="bg-white rounded-2xl shadow-sm p-4">
        <div className="flex items-center justify-between mb-4"><h3 className="text-lg font-semibold">Godziny - ostatnie 3 miesiące</h3><BarChart3 size={24} style={{color: colors.primary.medium}} /></div>
        <div className="flex items-center justify-between mb-4">
      ...
      <div className="bg-white rounded-2xl shadow-sm p-4">
        <div className="flex items-center justify-between mb-4"><h3 className="text-lg font-semibold">Podział wg stanowisk</h3><TrendingUp size={24} style={{color: colors.primary.medium}} /></div>
        <div className="flex items-center justify-between mb-4">
      ...
      {hourlyRate > 0 && (
        <div className="bg-white rounded-2xl shadow-sm p-4">
          <div className="flex items-center justify-between mb-4"><h3 className="text-lg font-semibold">Podsumowanie zarobków</h3><DollarSign size={24} style={{color: colors.accent.dark}} /></div>
          <div className="flex items-center justify-between mb-4">
          ...
          <div className="space-y-3">
            {monthsData.map((m, i) => (
              <div key={i} className="flex items-center justify-between py-2 border-b last:border-0">
                <div><p className="font-medium">{m.fullLabel}</p><p className="text-sm text-slate-500">{m.totalHours.toFixed(1)}h × {hourlyRate.toFixed(2)} zł</p></div>
                <span className="text-lg font-semibold" style={{color: colors.accent.dark}}>{m.earnings.toFixed(2)} zł</span>
            ...
            <div className="flex items-center justify-between pt-2 mt-2 border-t-2">
              ...
              <span className="text-xl font-bold" style={{color: colors.accent.dark}}>{totalEarnings3Months.toFixed(2)} zł</span>
      ...
      {hourlyRate === 0 && (
        <div className="rounded-2xl p-4 text-center" style={{backgroundColor: colors.accent.bg}}>
          <DollarSign size={32} style={{color: colors.accent.medium}} className="mx-auto mb-2" />
          <p className="text-sm" style={{color: colors.accent.dark}}>Dodaj stawkę godzinową w zakładce "Dane użytkownika"</p>
        <div className="bg-amber-50 rounded-2xl p-4 text-center">
          <DollarSign size={32} className="text-amber-400 mx-auto mb-2" />
          <p className="text-amber-700 text-sm">Dodaj stawkę godzinową w zakładce "Dane użytkownika"</p>
        </div>
      )}
    </div>
  );
      ...
      <div className="bg-white rounded-2xl overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-cyan-500 to-cyan-600 p-6 text-center">
          <div className="w-20 h-20 bg-white rounded-full flex items-center justify-center mx-auto mb-3">
            <span className="font-bold text-2xl" style={{color: colors.primary.medium}}>{user.initials}</span>
          ...
          <p className="text-cyan-100 text-sm mt-1">Konto zabezpieczone</p>
        </div>
        <div className="p-4 space-y-4">
          <div><label className="block text-sm font-medium text-slate-600 mb-1">Imię i nazwisko</label><input value={form.name} onChange={(e) => setForm({...form, name: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl" placeholder="Imię i nazwisko" /></div>
          <div><label className="block text-sm font-medium text-slate-600 mb-1">Email</label><input value={form.email} disabled className="w-full p-3 bg-slate-100 rounded-xl text-slate-500" /></div>
          ...
          <button onClick={save} className={`w-full py-3 rounded-xl font-semibold ${saved ? 'bg-green-500 text-white' : 'bg-cyan-500 text-white'}`}>{saved ? '✓ Zapisano' : 'Zapisz zmiany'}</button>
        </div>
      </div>
    </div>
  );
  ...
  <div className="min-h-screen bg-slate-50 p-4 pb-24">
    <div className="bg-white rounded-2xl overflow-hidden">
      ...
      <div className="bg-gradient-to-r from-cyan-500 to-cyan-600 p-8 text-center">
        ...
        <span className="text-white text-2xl font-light">REX <span className="text-cyan-200">Cloud</span></span>
        <p className="text-cyan-100 mt-2">v3.1.0 - Secure</p>
      </div>
      <div className="p-6 space-y-4">
        <div className="rounded-xl p-4" style={{backgroundColor: colors.primary.bg}}>
          <div className="flex items-center gap-2 mb-2"><Lock size={18} style={{color: colors.primary.dark}} /><span className="font-semibold" style={{color: colors.primary.darkest}}>Bezpieczeństwo</span></div>
          <ul className="text-sm space-y-1" style={{color: colors.primary.dark}}>
        <div className="bg-green-50 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-2"><Lock size={18} className="text-green-600" /><span className="font-semibold text-green-800">Bezpieczeństwo</span></div>
          <ul className="text-sm text-green-700 space-y-1">
            <li>• Hasła hashowane SHA-256</li>
            <li>• Dane zapisywane lokalnie</li>
            <li>• Automatyczna synchronizacja</li>
          </ul>
        </div>
        <div className="bg-cyan-50 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-2"><AlertCircle size={18} className="text-cyan-600" /><span className="font-semibold text-cyan-800">Wnioski</span></div>
          <p className="text-sm text-cyan-700">Wnioski o zmiany oczekują teraz na zatwierdzenie przez administratora</p>
        </div>
...
function REXCloudApp() {
  ...
  // AUTOMATYCZNA DATA - używa bieżącego miesiąca
  const [date, setDate] = useState(() => new Date());
  
  const [user, setUser] = useState(null);
  const [shifts, setShifts] = useState([]);
  const [requests, setRequests] = useState(() => loadFromStorage('shift_requests', [])); // Wnioski o zmiany
  ...
  // Zapisz wnioski do localStorage
  useEffect(() => {
    saveToStorage('shift_requests', requests);
  }, [requests]);
  
  // USUNIĘTO auto-save przy zmianach z wniosków - teraz admin zarządza zmianami
  ...
  // ZAKTUALIZOWANE - znajdź najbliższą zmianę od TERAZ
  const today = new Date();
  const todayStr = today.toISOString().split('T')[0];
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t px-4 py-2 flex justify-around z-10">
        {[['home', Home, 'Home'], ['shifts', Calendar, 'Zmiany'], ['preferences', Plus, 'Wnioski'], ['holidays', Umbrella, 'Urlopy']].map(([id, Icon, label]) => (
          <button key={id} onClick={() => setPage(id)} className="flex flex-col items-center p-2" style={{color: page === id ? colors.primary.medium : '#94a3b8'}}>
