...
const positionColors = { 
  'KIT': '#059669',  // Emerald - Kuchnia (stonowany zielony)
  'CAS': '#6366F1',  // Indigo - Kasa (szafirowy)
  'SUP': '#DC2626',  // Red - Wsparcie (stonowany czerwony)
  'RUN': '#7C3AED',  // Violet - Runner (fioletowy)
  'SIN': '#EA580C',  // Orange - Sink (ciepły pomarańcz)
  'LOB': '#64748B'   // Slate - Lobby (elegancki szary)
};
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-800 to-slate-900 flex items-center justify-center p-6">
      <div className="w-full max-w-sm">
        <div className="flex items-center justify-center gap-3 mb-12">
          <div className="w-14 h-14 bg-indigo-500 rounded-xl flex items-center justify-center"><Cloud size={32} className="text-white" /></div>
          <div><span className="text-white text-3xl font-light">REX</span><span className="text-indigo-300 text-3xl font-light ml-2">Cloud</span></div>
        </div>
        <div className="bg-white rounded-2xl p-8">
          <div className="flex items-center justify-center gap-2 mb-6"><Lock size={20} className="text-indigo-500" /><h2 className="text-2xl font-semibold">Zaloguj się</h2></div>
          {error && <div className="bg-red-50 text-red-600 p-3 rounded-lg mb-4 text-sm">{error}</div>}
          <div className="space-y-4">
            <div><label className="block text-sm text-slate-600 mb-1">Email</label><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-4 py-3 rounded-xl border focus:border-indigo-500 outline-none" placeholder="Email" disabled={loading} /></div>
            <div><label className="block text-sm text-slate-600 mb-1">PIN</label><input type="password" value={pin} onChange={(e) => setPin(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleLogin()} className="w-full px-4 py-3 rounded-xl border focus:border-indigo-500 outline-none" placeholder="••••" maxLength={4} disabled={loading} /></div>
            <button onClick={handleLogin} disabled={loading} className="w-full bg-indigo-500 hover:bg-indigo-600 disabled:bg-indigo-300 text-white font-semibold py-3 rounded-xl transition-colors">{loading ? 'Logowanie...' : 'Zaloguj się'}</button>
          </div>
...
const Sidebar = ({ isOpen, onClose, currentPage, onNavigate, user, onLogout }) => {
  ...
  return (<>{isOpen && <div className="fixed inset-0 bg-black/50 z-40" onClick={onClose} />}<div className={`fixed top-0 left-0 h-full w-72 bg-white z-50 transform transition-transform flex flex-col ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}><div className="bg-gradient-to-r from-indigo-500 to-indigo-600 p-4 pt-8"><div className="flex items-center gap-2 mb-4"><Cloud size={24} className="text-white" /><span className="text-white text-lg font-light">REX <span className="text-indigo-200">Cloud</span></span></div></div><div className="p-4 border-b flex items-center gap-3"><div className="w-12 h-12 bg-indigo-500 rounded-full flex items-center justify-center text-white font-semibold">{user.initials}</div><div><p className="font-semibold text-sm">{user.name}</p><p className="text-slate-500 text-xs">{user.email}</p></div></div><nav className="p-4 flex-1">{items.map(item => (<button key={item.id} onClick={() => { onNavigate(item.id); onClose(); }} className={`w-full flex items-center gap-4 px-4 py-3 rounded-xl ${currentPage === item.id ? 'bg-indigo-50 text-indigo-600' : 'text-slate-600'}`}><item.icon size={20} /><span className="font-medium">{item.label}</span></button>))}</nav><div className="p-4 border-t"><button onClick={() => { onLogout(); onClose(); }} className="w-full flex items-center gap-4 px-4 py-3 rounded-xl text-red-600"><LogOut size={20} /><span className="font-medium">Wyloguj się</span></button></div></div></>);
};

const Header = ({ title, onMenuClick }) => (<div className="bg-gradient-to-r from-indigo-500 to-indigo-600 text-white px-4 py-4 flex items-center justify-between sticky top-0 z-30"><div className="flex items-center gap-3"><Cloud size={24} /><span className="text-lg font-medium">{title}</span></div><button onClick={onMenuClick} className="p-2"><Menu size={24} /></button></div>);
...
const SwipeableShiftCard = ({ shift, onShowCoworkers, isRevealed, onReveal, isSelected, onSelect }) => {
  ...
  return (
    <div className="relative overflow-hidden rounded-xl shadow-sm mb-3">
      ...
      <div className="absolute right-0 top-0 bottom-0 w-20 bg-indigo-500 flex items-center justify-center rounded-r-xl">
        <button 
      ...
      {/* Main card */}
      <div 
        className={`relative border-l-4 border-indigo-400 p-4 transition-all duration-200 ${isHighlighted ? 'bg-indigo-50' : 'bg-white'}`}
      ...
      >
        <div className="flex gap-4">
          <div className={`rounded-xl px-3 py-2 text-center min-w-14 ${isHighlighted ? 'bg-indigo-100' : 'bg-indigo-50'}`}>
            <p className="text-xs text-indigo-600">{shift.dayName}</p>
                ...
                {coworkers.map((cw, i) => (
                  <div key={i} className="flex items-center gap-3 py-2">
                    ...
                    <div>
                      ...
                      <p className="text-indigo-500 text-sm">{cw.time}</p>
                    </div>
                  </div>
                ))}
              </div>
...
const HomePage = ({ nextShift, onNavigateToShifts, vacation, onNavigateToHolidays }) => {
  ...
  return (<div className="p-4 space-y-4 pb-24"><div className="bg-white rounded-2xl shadow-sm border-l-4 border-indigo-500 p-4 cursor-pointer" onClick={onNavigateToShifts}><div className="flex items-center justify-between mb-4"><h3 className="text-lg font-semibold">Następna zmiana</h3><Calendar size={24} className="text-indigo-500" /></div>{nextShift ? (<div className="flex gap-4"><div className="bg-slate-50 rounded-xl p-3 text-center min-w-16"><p className="text-sm text-slate-500">{nextShift.dayName}</p><p className="text-3xl font-bold">{nextShift.dayNum}</p><p className="text-sm text-slate-500">{monthNames[new Date(nextShift.date).getMonth()].substring(0,3).toUpperCase()}</p></div><div className="flex-1">{nextShift.shifts.map((s, i) => (<div key={i} className="flex items-center gap-2 mb-1"><Clock size={16} className="text-slate-400" /><span>{s.time}</span><span className="text-sm px-2 py-0.5 rounded" style={{backgroundColor: s.color+'20', color: s.color}}>{s.type}</span></div>))}<p className="text-slate-500 text-sm mt-1">{nextShift.location}</p><div className="flex gap-4 mt-4 pt-3 border-t"><div className="text-center"><p className="text-2xl font-bold text-indigo-500">{shiftCountdown.days}</p><p className="text-xs text-slate-500">Dni</p></div><div className="text-center"><p className="text-2xl font-bold">{shiftCountdown.hours}</p><p className="text-xs text-slate-500">godz</p></div><div className="text-center"><p className="text-2xl font-bold">{shiftCountdown.min}</p><p className="text-xs text-slate-500">min</p></div></div></div></div>) : (<div className="text-center py-4"><Cloud size={40} className="text-slate-200 mx-auto mb-2" /><p className="text-slate-500">Brak zaplanowanych zmian</p></div>)}</div><div className="bg-white rounded-2xl shadow-sm border-l-4 border-orange-400 p-4 cursor-pointer" onClick={onNavigateToHolidays}><div className="flex items-center justify-between mb-4"><h3 className="text-lg font-semibold">Następny urlop</h3><Umbrella size={24} className="text-orange-500" /></div>{vacation ? (<div className="flex gap-4"><div className="bg-orange-50 rounded-xl p-3 text-center min-w-16"><p className="text-sm text-orange-600">{vacDayNames[vacDate.getDay()]}</p><p className="text-3xl font-bold">{vacDate.getDate()}</p><p className="text-sm text-orange-600">{monthNames[vacDate.getMonth()].substring(0,3).toUpperCase()}</p></div><div className="flex-1"><div className="flex items-center gap-2 mb-1"><Umbrella size={16} className="text-orange-500" /><span className="font-medium">{vacation.type}</span></div><div className="flex items-center gap-2 text-slate-600 text-sm"><Calendar size={14} className="text-slate-400" /><span>{formatDate(vacDate)} - {formatDate(vacEndDate)}</span></div><p className="text-orange-600 text-sm font-medium mt-1">Łącznie: {vacation.totalDays} dni</p><div className="flex gap-4 mt-4 pt-3 border-t"><div className="text-center"><p className="text-2xl font-bold text-orange-500">{vacCountdown.days}</p><p className="text-xs text-slate-500">Dni</p></div><div className="text-center"><p className="text-2xl font-bold">{vacCountdown.hours}</p><p className="text-xs text-slate-500">godz</p></div><div className="text-center"><p className="text-2xl font-bold">{vacCountdown.min}</p><p className="text-xs text-slate-500">min</p></div></div></div></div>) : (<div className="text-center py-4"><Umbrella size={40} className="text-orange-200 mx-auto mb-2" /><p className="text-slate-500">Brak zaplanowanych urlopów</p></div>)}</div></div>);
};
...
const PreferencesPage = ({ date, onDateChange, onAddShift }) => {
  ...
  return (<div className="flex flex-col min-h-screen bg-slate-50 pb-20"><div className="bg-white px-4 py-4 border-b flex items-center justify-between"><button onClick={() => onDateChange(new Date(date.getFullYear(), date.getMonth()-1, 1))} className="p-2"><ChevronLeft size={24} /></button><span className="font-semibold">{monthNames[date.getMonth()]} {date.getFullYear()}</span><button onClick={() => onDateChange(new Date(date.getFullYear(), date.getMonth()+1, 1))} className="p-2"><ChevronRight size={24} /></button></div><div className="p-4"><button onClick={() => { setShowModal(true); setError(''); }} className="flex items-center gap-2 text-indigo-600 font-medium mb-4"><Plus size={20} /><span className="underline">Nowy wniosek o zmianę</span></button>{prefs.length === 0 ? (<div className="text-center py-12"><Cloud size={48} className="text-slate-300 mx-auto mb-4" /><p className="text-slate-500">Brak wniosków</p></div>) : prefs.map(p => (<div key={p.id} className="bg-white rounded-xl shadow-sm border-l-4 border-indigo-400 p-4 mb-3"><div className="flex items-center gap-4"><div className="bg-indigo-50 rounded-xl px-3 py-2 text-center min-w-14"><p className="text-xs text-indigo-600">{p.dayName}</p><p className="text-xl font-bold">{p.dayNum}</p></div><div><p className="font-medium">{p.type}</p>{p.position && <p className="text-sm text-slate-600">{p.position}</p>}{(p.timeFrom || p.timeTo) && <p className="text-sm text-slate-500">{p.timeFrom && 'od '+p.timeFrom} {p.timeTo && 'do '+p.timeTo}</p>}</div></div></div>))}</div>{showModal && (<div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center"><div className="bg-white w-full max-w-lg rounded-t-3xl p-6 max-h-[90vh] overflow-y-auto"><h3 className="text-xl font-semibold mb-6">Nowy wniosek o zmianę</h3>
  {error && <div className="bg-red-50 text-red-600 p-3 rounded-lg mb-4 text-sm">{error}</div>}
  <div className="space-y-4"><div><label className="block text-sm font-medium text-indigo-600 mb-2">Data</label><input type="date" min={todayStr} value={newPref.date} onChange={(e) => setNewPref({...newPref, date: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl" /></div><div><label className="block text-sm font-medium text-indigo-600 mb-2">Rodzaj</label><select value={newPref.type} onChange={(e) => setNewPref({...newPref, type: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl">{types.map(t => <option key={t} value={t}>{t}</option>)}</select></div>{needsFrom && (<div><label className="block text-sm font-medium text-indigo-600 mb-2">Od godziny</label><input type="time" value={newPref.timeFrom} onChange={(e) => setNewPref({...newPref, timeFrom: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl" /></div>)}{needsTo && (<div><label className="block text-sm font-medium text-indigo-600 mb-2">Do godziny</label><input type="time" value={newPref.timeTo} onChange={(e) => setNewPref({...newPref, timeTo: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl" /></div>)}{needsPos && (<div><label className="block text-sm font-medium text-indigo-600 mb-2">Pozycja</label><select value={newPref.position} onChange={(e) => setNewPref({...newPref, position: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl">{positions.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></div>)}<div className="bg-green-50 p-3 rounded-xl text-sm text-green-700">✓ Zmiana zostanie automatycznie zsynchronizowana</div></div><div className="flex gap-3 mt-6"><button onClick={() => setShowModal(false)} className="flex-1 py-3 bg-slate-100 rounded-xl font-semibold">Anuluj</button><button onClick={handleAdd} className="flex-1 py-3 bg-indigo-500 text-white rounded-xl font-semibold">Dodaj zmianę</button></div></div></div>)}</div>);
};
...
const HolidaysPage = ({ vacation, onAddVacation }) => {
  ...
  return (<div className="flex flex-col min-h-screen bg-slate-50 pb-24"><div className="p-4"><button onClick={() => setShowModal(true)} className="flex items-center gap-2 text-orange-600 font-medium mb-4"><Plus size={20} /><span className="underline">Dodaj nowy urlop</span></button>{vacation ? (<div className="bg-white rounded-2xl shadow-sm border-l-4 border-orange-400 p-4"><div className="flex items-center justify-between mb-4"><h3 className="text-lg font-semibold">Zaplanowany urlop</h3><Umbrella size={24} className="text-orange-500" /></div><div className="flex gap-4"><div className="bg-orange-50 rounded-xl p-3 text-center min-w-16"><p className="text-sm text-orange-600">{vacDayNames[vacDate.getDay()]}</p><p className="text-3xl font-bold">{vacDate.getDate()}</p><p className="text-sm text-orange-600">{monthNames[vacDate.getMonth()].substring(0,3).toUpperCase()}</p></div><div className="flex-1"><div className="flex items-center gap-2 mb-1"><Umbrella size={16} className="text-orange-500" /><span className="font-medium">{vacation.type}</span></div><div className="flex items-center gap-2 text-slate-600 text-sm"><Calendar size={14} className="text-slate-400" /><span>{formatDate(vacDate)} - {formatDate(vacEndDate)}</span></div><p className="text-orange-600 text-sm font-medium mt-1">Łącznie: {vacation.totalDays} dni</p><div className="flex gap-4 mt-4 pt-3 border-t"><div className="text-center"><p className="text-2xl font-bold text-orange-500">{vacCountdown.days}</p><p className="text-xs text-slate-500">Dni</p></div><div className="text-center"><p className="text-2xl font-bold">{vacCountdown.hours}</p><p className="text-xs text-slate-500">godz</p></div><div className="text-center"><p className="text-2xl font-bold">{vacCountdown.min}</p><p className="text-xs text-slate-500">min</p></div></div></div></div></div>) : (<div className="bg-white rounded-2xl shadow-sm p-8 text-center"><Umbrella size={64} className="text-orange-200 mx-auto mb-4" /><p className="text-slate-500 mb-4">Brak zaplanowanych urlopów</p><button onClick={() => setShowModal(true)} className="px-6 py-3 bg-orange-500 text-white font-semibold rounded-xl">Złóż wniosek</button></div>)}</div>{showModal && (<div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center"><div className="bg-white w-full max-w-lg rounded-t-3xl p-6"><h3 className="text-xl font-semibold mb-6">Nowy wniosek urlopowy</h3><div className="space-y-4"><div><label className="block text-sm font-medium text-orange-600 mb-2">Typ urlopu</label><select value={newVac.type} onChange={(e) => setNewVac({...newVac, type: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl">{vacTypes.map(t => <option key={t} value={t}>{t}</option>)}</select></div><div><label className="block text-sm font-medium text-orange-600 mb-2">Data rozpoczęcia</label><input type="date" min={todayStr} value={newVac.startDate} onChange={(e) => setNewVac({...newVac, startDate: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl" /></div><div><label className="block text-sm font-medium text-orange-600 mb-2">Data zakończenia</label><input type="date" min={newVac.startDate || todayStr} value={newVac.endDate} onChange={(e) => setNewVac({...newVac, endDate: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl" /></div>{newVac.startDate && newVac.endDate && (<div className="bg-orange-50 p-3 rounded-xl text-sm text-orange-700">Łączna liczba dni: <strong>{Math.ceil((new Date(newVac.endDate) - new Date(newVac.startDate)) / 86400000) + 1}</strong></div>)}</div><div className="flex gap-3 mt-6"><button onClick={() => setShowModal(false)} className="flex-1 py-3 bg-slate-100 rounded-xl font-semibold">Anuluj</button><button onClick={handleAdd} className="flex-1 py-3 bg-orange-500 text-white rounded-xl font-semibold">Złóż wniosek</button></div></div></div>)}</div>);
};
...
const StatisticsPage = ({ shifts, hourlyRate = 0 }) => {
  ...
  return (<div className="min-h-screen bg-slate-50 p-4 pb-24 space-y-4"><div className="bg-white rounded-2xl shadow-sm p-4"><div className="flex items-center justify-between mb-4"><h3 className="text-lg font-semibold">{currentMonthData.fullLabel} {currentMonthData.year}</h3><Clock size={24} className="text-indigo-500" /></div><div className="grid grid-cols-2 gap-4"><div className="bg-indigo-50 rounded-xl p-4 text-center"><p className="text-3xl font-bold text-indigo-600">{currentMonthData.totalHours.toFixed(1)}</p><p className="text-sm text-indigo-700">godzin</p></div><div className="bg-emerald-50 rounded-xl p-4 text-center"><p className="text-3xl font-bold text-emerald-600">{currentMonthData.shiftsCount}</p><p className="text-sm text-emerald-700">zmian</p></div></div>{hourlyRate > 0 && (<div className="mt-4 bg-orange-50 rounded-xl p-4"><div className="flex items-center justify-between"><div className="flex items-center gap-2"><DollarSign size={20} className="text-orange-600" /><span className="text-sm text-orange-700">Prognozowane wynagrodzenie</span></div><span className="text-xl font-bold text-orange-600">{currentMonthData.earnings.toFixed(2)} zł</span></div></div>)}</div><div className="bg-white rounded-2xl shadow-sm p-4"><div className="flex items-center justify-between mb-4"><h3 className="text-lg font-semibold">Godziny - ostatnie 3 miesiące</h3><BarChart3 size={24} className="text-indigo-500" /></div><BarChart data={chartData} maxValue={maxChartValue} /><div className="mt-4 pt-4 border-t flex justify-between text-sm"><div><span className="text-slate-500">Łącznie:</span><span className="font-semibold ml-2">{totalHours3Months.toFixed(1)}h</span></div><div><span className="text-slate-500">Średnia/mies.:</span><span className="font-semibold ml-2">{avgHoursPerMonth.toFixed(1)}h</span></div></div></div><div className="bg-white rounded-2xl shadow-sm p-4"><div className="flex items-center justify-between mb-4"><h3 className="text-lg font-semibold">Podział wg stanowisk</h3><TrendingUp size={24} className="text-indigo-500" /></div><p className="text-sm text-slate-500 mb-3">{currentMonthData.fullLabel}</p>{currentMonthData.totalHours > 0 ? <PositionBreakdown positions={currentMonthData.positions} /> : <p className="text-slate-400 text-center py-4">Brak danych</p>}</div>{hourlyRate > 0 && (<div className="bg-white rounded-2xl shadow-sm p-4"><div className="flex items-center justify-between mb-4"><h3 className="text-lg font-semibold">Podsumowanie zarobków</h3><DollarSign size={24} className="text-emerald-500" /></div><div className="space-y-3">{monthsData.map((m, i) => (<div key={i} className="flex items-center justify-between py-2 border-b last:border-0"><div><p className="font-medium">{m.fullLabel}</p><p className="text-sm text-slate-500">{m.totalHours.toFixed(1)}h × {hourlyRate.toFixed(2)} zł</p></div><span className="text-lg font-semibold text-emerald-600">{m.earnings.toFixed(2)} zł</span></div>))}<div className="flex items-center justify-between pt-2 mt-2 border-t-2"><span className="font-semibold">Razem (3 mies.)</span><span className="text-xl font-bold text-emerald-600">{totalEarnings3Months.toFixed(2)} zł</span></div></div></div>)}{hourlyRate === 0 && (<div className="bg-orange-50 rounded-2xl p-4 text-center"><DollarSign size={32} className="text-orange-400 mx-auto mb-2" /><p className="text-orange-700 text-sm">Dodaj stawkę godzinową w zakładce "Dane użytkownika"</p></div>)}</div>);
};
...
function REXCloudApp() {
  ...
  if (!user) return <div className="min-h-screen bg-slate-50 flex items-center justify-center"><Cloud size={48} className="text-indigo-500 animate-pulse" /></div>;
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t px-4 py-2 flex justify-around z-10">
        {[['home', Home, 'Home'], ['shifts', Calendar, 'Zmiany'], ['preferences', Plus, 'Wnioski'], ['holidays', Umbrella, 'Urlopy']].map(([id, Icon, label]) => (
          <button key={id} onClick={() => setPage(id)} className={`flex flex-col items-center p-2 ${page === id ? 'text-indigo-500' : 'text-slate-400'}`}>
